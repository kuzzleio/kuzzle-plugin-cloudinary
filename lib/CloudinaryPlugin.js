...
class CloudinaryPlugin {
  ...
  async addTag(request) {
    const tag = this._getArg(request.input.body, 'tag'),
      ...
      res = await Cloudinary.uploader.addTag(tag, to);
  ...
  async replaceTag(request) {
    const tag = this._getArg(request.input.body, 'tag'),
      to = this._getArg(request.input.body, 'public_id', true),
      res = await Cloudinary.uploader.replaceTag(tag, to);
    this._handleError(to, res.public_ids);
    return res;
  }
  ...
  async removeTag(request) {
    const tag = this._getArg(request.input.body, 'tag'),
      to = this._getArg(request.input.body, 'public_id', true),
      res = await Cloudinary.uploader.removeTag(tag, to);
    this._handleError(to, res.public_ids);
    return res;
  }
  ...
  async removeAllTags(request) {
    const to = this._getArg(request.input.body, 'public_id', true),
      res = await Cloudinary.uploader.removeAllTags(to);
    this._handleError(to, res.public_ids);
    return res;
  }
