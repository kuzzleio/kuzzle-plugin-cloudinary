...
class CloudinaryPlugin {
  ...
  async rename(request) {
    const from = this._getArg(request, 'from_public_id'),
      to = this._getArg(request, 'to_public_id');
    try {
  ...
  async add_tag(request) {
    const tag = this._getArg(request, 'tag'),
      to = this._getArrayArg(request, 'public_id'),
