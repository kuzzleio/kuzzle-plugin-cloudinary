...
class CloudinaryPlugin {
  ...
  async rename(request) {
    const from = this._getArg(request.input.body, 'from_public_id'), to = this._getArg(request.input.body, 'to_public_id');
    try {
  ...
  async add_tag(request) {
    const tag = this._getArg(request.input.body, 'tag'), to = this._getArg(request.input.body, 'public_id', true);
  ...
  async transform(request) {
    console.log(request);
    //const public_id = this._getArg(request, 'public_id'), transform = this._getArg(request, 'transformation');
    //console.log(await Cloudinary.url(public_id, transform));
  }
  ...
  _handleError(givenPublicIDS, resultPublicIDS) {
    if (givenPublicIDS.length !== resultPublicIDS.length) {
      ...
      throw new this.context.errors.PartialError(
        'Some ressources had not been updated',
